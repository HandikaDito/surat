...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('transaction')->as('transaction.')->group(function () {
        ...
        // ✅ APPROVE (RAPI)
        Route::post('{letter}/disposition/{disposition}/approve', [\App\Http\Controllers\DispositionController::class, 'approve'])
            ->name('transaction.disposition.approve')
            ->middleware('role:pimpinan');

        // 🔹 DOWNLOAD ATTACHMENTS
        Route::get('incoming-attachments/download/{attachment}', [\App\Http\Controllers\IncomingLetterController::class, 'downloadAttachment'])
            ->name('incoming-attachments.download');
        Route::get('outgoing-attachments/download/{attachment}', [\App\Http\Controllers\OutgoingLetterController::class, 'downloadAttachment'])
            ->name('outgoing-attachments.download');
    });
